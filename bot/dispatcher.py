...
class CommandDispatcher:
    ...
    def dispatch(self, message: BotMessage) -> BotResponse:
        ...
        try:
            ...
            return asyncio.run(self.dispatch_async(message))
        ...
        def _runner() -> None:
            try:
                result_holder["response"] = asyncio.run(self.dispatch_async(message))
...
"""
    ...
    async def _parse_intent_via_llm(text: str, config) -> Optional[dict]:
        ...
        try:
            ...
            logger.debug("[Dispatcher] NL parse: invalid JSON from LLM: %s", raw[:200] if 'raw' in locals() else "N/A")
            return None
