...
class Config:
    ...
    # 实时行情数据源优先级（逗号分隔）
    # 推荐顺序：tencent > efinance > tushare（AKShare已移除）
    # - tencent: 腾讯财经，有量比/换手率/市盈率等，单股查询稳定（推荐）
    # - efinance: 东财全量接口，数据最全但容易被封（AKShare已移除）
    # - tushare: Tushare Pro，需要2000积分，数据全面（付费用户可优先使用）
    realtime_source_priority: str = "tencent,efinance,tushare"
    # 实时行情缓存时间（秒）
    ...
    # === 流控配置（防封禁关键参数）===
    # 请求间隔范围（秒，AKShare已移除）
    request_sleep_min: float = 2.0
    request_sleep_max: float = 5.0
    ...
    def _resolve_realtime_source_priority(cls) -> str:
        ...
        default_priority = 'tencent,efinance,tushare'
