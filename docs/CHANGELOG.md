## [Unreleased]
- [修复] `scripts/backtest_5y_23strategies.py` 修复 ensemble 6 组件（ma_crossover/wave_theory/limit_up_pullback/rsi_bullish_divergence/fc_pos_break/holder_conc_break）上报指标口径不一致：组件在共振过滤前回测并写入结果表（用于计算 walk-forward 权重），共振过滤后其信号被大幅削减（如 limit_up_pullback 冷却后 1244→共振后约 170 笔）但结果表仍沿用共振前旧指标（830 笔/98.7% 总收益），导致汇总表/排名/5 日验证/handoff 与双口径对比全部失真（双口径对比曾出现 limit_up_pullback 总收益 close 39.9%→next_open 98.7% 的 +58.8pct 假差异）；现共振后用共振后信号重算组件指标并替换结果条目，组件权重仍基于共振前全样本历史（权重口径不变）。修复后 limit_up_pullback 上报 174 笔/48.3% 胜率/38.0% 总收益，双口径差异回落至 -1.9pct 正常水平。
- [改进] `scripts/backtest_5y_23strategies.py` WFO 阈值扫描复评后收紧两个触发阈值：(1) `sig_wonderful_9_turn` 阈值由 8 回收为 10（8 在全量回测中超发为 26 笔/30.8% 胜率/-0.507 期望；无共振扫描中阈值 10 稳定性 0.67 与 8/9 并列、全期期望 1.262%/胜率 54.7% 显著优于 8 的 0.464%/46.7% 与 9 的 0.517%/47.4%），同步更新 WFO 扫描注册表基准值为 10；(2) ensemble 连续强度加权触发阈值 `ENSEMBLE_MIN_WEIGHTED_SCORE` 由 0.35 提高为 0.45（0.35/0.45/0.55/0.65 扫描中 0.45 稳定性 1.00、全期期望 1.527%/胜率 47.3% 均为最优，生产口径约 287 笔/全期），`--ensemble-classic` 旧 3 组件行为不变。
- [新功能] `scripts/backtest_5y_23strategies.py` `--wfo-sweep` 扫描范围扩展：新增 wonderful_9_turn 阈值扫描（显式覆盖 8/9/10）与 ensemble 连续强度触发阈值扫描 0.35/0.45/0.55/0.65（新增 `sweep_ensemble_thresholds` helper，复用组件冷却后信号回测与 `_build_walkforward_weights` 季度权重口径，扫描期间临时改写全局阈值/权重并在结束后恢复；单策略扫描无法施加共振过滤，信号量偏高、阈值排序有效）。
- [新功能] `scripts/backtest_5y_23strategies.py` 新增 `--no-handoff` 开关：回测结果表与 5 日验证输出后即退出，跳过 handoff 评分选股与 `main.py` LLM 分析子进程（批量 A/B 回测用）；默认仍执行 handoff，旧行为不变。
- [新功能] `scripts/backtest_5y_23strategies.py` 动态退出新增移动止盈激活门槛与固定止盈（均默认关闭、A/B 可选）：`compute_dynamic_exit_returns` 新增 `trail_activate_r`（R 倍数，浮盈达到 `trail_activate_r×(atr_mult×atr20入场值)` 后移动止盈才武装，武装前仅 ATR 止损/时间止损可出场，0.0=入场即武装旧行为）与 `take_profit_r`（R 倍数，bar 最高价触及 `entry+take_profit_r×初始风险` 即按止盈价成交，0.0=关闭）；同 bar 触发优先级 ATR 止损 > 固定止盈 > 移动止盈（保守口径），双入场口径（close/next_open）均生效，固定止盈/移动止盈/止损在跌停 bar 仍按既有规则顺延（涨停 bar 不阻断止盈成交）；新增 CLI `--trail-activate-r`/`--take-profit-r`（默认 0.0），经模块全局 `_TRAIL_ACTIVATE_R`/`_TAKE_PROFIT_R` 透传主调用与 `--per-strategy-exit` 分组调用；默认参数与旧实现逐位等价（合成回归测试验证）。
- [新功能] `scripts/backtest_5y_23strategies.py` 组合级波动率目标仓位（默认关闭）：`calc_portfolio_metrics` 新增 `target_vol_annual`/`max_leverage`，启用后每日总敞口 `g_t=clip(目标年化波动/已实现组合年化波动, 0, max_leverage)`，已实现波动用截至前一日的 20 日滚动等权组合日收益 std×√252（causal 无前视，历史不足或波动≈0 时 g=1.0），缩放日收益=port_daily×g_t，仅影响组合级总收益/年化/最大回撤/夏普并日志输出平均总敞口，逐笔胜率/盈亏比/期望/凯利不变；新增 CLI `--target-vol`（默认 0.0=等权旧口径）/`--max-leverage`（默认 1.0 不加杠杆），经模块全局 `_TARGET_VOL_ANNUAL`/`_MAX_LEVERAGE` 透传 `calc_metrics`。
- [新功能] `scripts/backtest_5y_23strategies.py` 新增 WFO 阈值扫描脚手架 `--wfo-sweep`（默认关闭）：对 6 个高流动性策略（ma_crossover/wave_theory/limit_up_pullback/monthly_macd_20ma/n_pattern/volume_breakout）的打分阈值各做 {-1,0,+1} 点 walk-forward 扫描，原始打分序列只算一次、每个变体仅重算该策略布尔掩码并复用 `_backtest_single`（不重算指标/出场，复用暖缓存分钟级完成），按自然年度桶报告逐年胜率/期望、全期期望与稳定性（期望为正的年份占比），输出紧凑排序表；新增可导入 helper `sweep_strategy_thresholds(df, strategy_names, deltas, entry_timing, enhanced)` 与 `report_wfo_sweep(rows)`，并提取 `_volume_breakout_score` 供阈值复用（sig 判定不变）；该分支跳过全量 23 策略回测/验证/handoff，仅为未来优化的可复现脚手架，不自动改配置/不自动交易。
- [新功能] `scripts/backtest_5y_23strategies.py` 回测 P1 批次2：市场环境新增 `market_ok_enh2` 口径（`market_ok_enh` 再叠加市场广度 close>ma20 占比≥0.45 与指数 20 日年化波动率扩展分位≤0.85 双重确认），`compute_market_ok` 新增 `stock_df` 参数按日向量化聚合广度并记录 enh/enh2 可开仓天数占比；新增 `--regime-mode {enh2,enh,strict}`（默认 enh2，enh=旧 market_ok_enh，strict=旧 market_ok；`--strict` 等价 strict），`_entry_mask`/验证区间市况统计/handoff 统一按该口径取列。
- [新功能] `scripts/backtest_5y_23strategies.py` handoff 硬门槛改为共振强度评分（`--handoff-mode {score,gate}`，默认 score，gate=旧硬门槛）：每个命中策略按 √(笔数/30)（封顶 1，小样本部分计分、永不为 0）×（全样本期望>0 ? 1.0 : 0.3）×（该股验证窗收益>0 ? 1.0 : 0.5）贡献权重，按总分降序取 score>0 前 top_n 送入主程序，并打印 code/名称/评分/命中数/稳健数/验证收益排名表；旧门槛下被丢弃的股票现在以低分出现，仅当全宇宙无任何信号（score 全 0）才跳过主程序。
- [新功能] `scripts/backtest_5y_23strategies.py` 5 日验证重设计：5 个交易日均作为入场日（旧仅第 5 天单日买入），每笔按策略最优持有期读取预计算 dyn_ret 前瞻收益（与回测同一套 ATR 止损/移动止盈/时间止损+跌停顺延真实出场规则），出场窗口超出数据记为“未平仓”单独计数且不计胜负；候选策略改为回测期望前 8 且样本充足（≥30 笔），输出表新增“未平仓”列。
- [改进] `scripts/backtest_5y_23strategies.py` 组合策略 ensemble 由 3 组件二值投票扩展为 6 组件连续强度（ma_crossover/wave_theory/limit_up_pullback 趋势突破 + rsi_bullish_divergence 反转 + fc_pos_break/holder_conc_break 基本面锚）：打分制组件贡献 raw_score/触发阈值截断 [0,1.5] 的连续强度、布尔组件命中贡献 1.0，加权和≥0.35 触发；walk-forward 季度权重扩展到 6 组件（≤10 笔/期望≤0 排除与等权回退不变）；新增 `--ensemble-classic` 回退旧 3 组件二值+0.5 阈值。
- [改进] `scripts/backtest_5y_23strategies.py` bear 日弱信号软过滤确认数由 1 收紧为 2（同股同日需≥2 个全市场锚或 bear 族强势命中才保留趋势/动量弱信号），新增 `--bear-confirm N`（默认 2，旧行为传 1）。
- [修复] `scripts/backtest_5y_23strategies.py` 修复自相矛盾/近零信号策略：(1) `emotion_cycle` 质量族由 standard（要求获利盘≥0.2，与 RSI6<35 超卖互斥）改为 washout（换手率带+质押+股息，无获利盘下限）；(2) `sig_chan_theory` 底背驰重写为价格创 10 日新低且 MACD DIF 不低于前 20 根 DIF 谷底（旧仅比较 dif 与昨 dif 的 1 日 uptick，非真背驰）+量能+RSI6<55+阳线；(3) `sig_low_position_limit_up` 放松为涨停且 close<0.95×60 日高点（旧 0.9×20 日高点与涨停近乎互斥）+换手≥3%（旧 5%）+价格<80（旧 50）+前 10 日无涨停（旧 20），并对 low_position_limit_up/stable_then_limit_up 豁免 `_entry_mask` 涨停买入掩码（策略本以涨停日为买点，次日开盘入场口径下可成交，跌停掩码仍生效）；(4) `sig_washout_break`/`sig_low_profit_hold` 获利盘上限 0.2→0.35 且收盘条件放宽为 close>ma20×0.98（捕捉突破时刻）；(5) `sig_wonderful_9_turn` 阈值 10→8，MA20 上穿 MA60 的矛盾打分项改为 MA20 上行或收盘≥ma20×0.98。
- [改进] `scripts/backtest_5y_23strategies.py` 结果表将 <30 笔小样本策略移至主表之后的“小样本策略（不参与排名/验证/handoff 权重满分）”独立分区，主表仅排名样本充足策略，报错策略单独列出；5 日验证候选与 Top-N 选取统一使用 ≥30 笔过滤列表。
- [修复] `scripts/backtest_5y_23strategies.py` 修复行业动量 PIT 映射的 `merge_asof` 排序键错误：`apply_industry_momentum` 中成分/行情两侧按 `[code, date]`/`[code, in_date]` 排序，但 `merge_asof(by=...)` 要求 on 键（date/in_date）全局单调，code 切换处 date 回跳触发 "left keys must be sorted"，异常被外层 fail-soft 吞掉导致行业 top-3 动量过滤静默全量放行（信号量约膨胀 3 倍、回测口径偏乐观）；改为两侧分别按 `in_date`/`date` 单键排序，全量回测验证警告消失、趋势策略笔数回落至历史可比量级。
- [修复] `scripts/backtest_5y_23strategies.py` 修复 handoff 稳健策略数（robust_strategy_count）永远为 0 的预存 bug：`get_top_stocks_by_win_rate` 构建 `stock_list` 字典时漏传 `total_trades_list`，导致共振评分模式“稳健”列恒为 0，且旧硬门槛模式（`--handoff-mode gate`）的“有效共振策略≥3”判定从未真正通过（日志恒报“有效共振策略0<3”）；补传该字段后稳健计数与 √(笔数/30) 评分口径一致。
...
## [Unreleased]
- [修复] `scripts/backtest_5y_23strategies.py` 回测前视偏差与可成交性修正（7 项）：(1) **入场时点默认改为次日开盘**——`--entry-timing` 默认值由 `close` 改为 `next_open`（信号日收盘买入不可成交，属前视偏差），`--entry-timing close` 可复现旧数字；动态退出收益改为一次扫描同时产出 close/next_open 两套列（`compute_dynamic_exit_returns(dual_timing=True)`），回测主口径用 next_open 并自动追加另一口径的逐策略对比日志（胜率/总收益 close→next_open 差值），无需第二次 30 分钟全量运行；ensemble 权重与推荐前瞻收益列（`get_top_stocks_by_win_rate`）同步使用主口径。(2) **出场侧跌停不可卖**——ATR 止损/移动止盈/时间止损触发在跌停 bar（复用入场侧 `_is_limit_down`：精确 `down_limit` 优先、板块 pct 阈值兜底）时，顺延到其后第一个非跌停 bar 按收盘价成交，窗口内再无非跌停 bar 则按最后 bar 收盘价退出（保守口径）。(3) **ensemble 权重改 walk-forward**——`_compute_ensemble_weights` 全样本权重保留兼容，新增 `_build_walkforward_weights` 按自然季度用"该季度起始日前已平仓交易"（平仓日按信号日+best_period 保守近似）的组件正期望归一化，单组件历史已平仓 ≤10 笔或期望 ≤0 排除、有效组件 <2 回退等权；`sig_ensemble` 按信号日查季度权重表，消除全样本权重应用于全样本的前视偏差。(4) **行业成分 point-in-time**——`load_industry_context` 加载 `in_date/out_date`，`apply_industry_momentum` 改 `merge_asof` 按 `in_date <= 当日` 取最新成分（out_date 已调出则失效），消除"纳入前即属该行业"前视；本地表 `out_date` 全空（仅当前成分），已加 warning 明示调出历史缺失、行业过滤可能偏乐观。(5) **准备态数据 parquet 缓存**——复权+增强信号合并后、指标计算前的 DataFrame 写 `data/cache/backtest_{start}_{end}_{CACHE_VERSION}.parquet`（v1，加载/合并逻辑变更需 bump），命中时跳过 PG 加载（~464s）+复权因子（~50s）+增强信号合并（~89s），新增 `--no-cache` 强制重算；指标/出场收益仍每次实时重算。(6) **内存检测修复**——`_detect_swap_memory_gb` 的 Windows ctypes 回退误把"物理内存+页面文件"提交限额当 swap（导致 swap≈物理内存、误报"RAM仅0.0GB，Swap补充63.7GB"），改为提交限额减物理内存；`_detect_available_memory_gb` 补充 ctypes 可用物理内存回退，并行模式决策使用真实物理内存。(7) **FutureWarning 修复**——`_merge_aux_by_date` 无事件分组补 NA 列时按 right 侧 dtype 对齐（datetime 列不再混入 all-NA float），并跳过空 piece，消除 concat 的 "empty or all-NA entries" 警告；顺带修复 `--confluence-only` help 文本未转义 `%` 导致 `--help` 崩溃的问题。
- [改进] `scripts/backtest_5y_23strategies.py` 回测质量门槛与健壮性增强（5 年 2021-01~2026-08 A/B 验证）：(1) **基本面合流过滤**（P1-3）新增 `--confluence-only`：纯技术策略信号需当日存在基本面催化剂（业绩预增 `forecast_pos`/筹码集中 `holder_chg<-3%`/机构龙虎榜净买 `inst_buy5>500万`）才保留，已内置基本面条件的 fc_pos_break/inst_smart_break/holder_conc_break/low_profit_hold/washout_break 豁免；实测信号量 -31%（3637→2501 笔）但平均胜率 49.8%→51.6%、每笔期望 1.315%→1.481%（+12.6%）、夏普 3.92→5.05（+29%）、回撤 7.2%→6.8%，为风险调整后收益最优的质量门槛。另加 `--confluence-regime {non_bull,bear}` 市况感知变体（仅弱市要求催化剂），实测胜率/期望回落至 baseline、劣于硬过滤，仅作实验保留。(2) **组合策略重构**（P1-2）：ensemble 组件由 ma_crossover/volume_surge_std/multi_ma_resonance 改为 ma_crossover/volume_surge_std/rsi_bullish_divergence（剔除期望最低的 multi_ma_resonance、换入低相关反转族），权重依据由总胜率改为每笔期望收益归一；ensemble 230→415 笔、胜率 47.4%→48.2%、总收益 50.9%→56.7%、夏普 2.78→3.22。(3) **市况软加权**（P1-1）新增 `--regime-soft`：基本面策略为全市场强势锚、bear 日趋势弱信号需强势确认，作为 `--regime-filter` 硬分族（信号 -61%）的软化替代；实测信号量 100% 保留但胜率/收益≈baseline，安全保留。(4) **numba 静默失效加固**（P1-4）：`@numba.njit(cache=True)` 改 `cache=False`，消除脚本经 runpy/importlib 以非 `__main__` 名加载时磁盘缓存错配（`No module named '<dynamic>'`）导致策略集体静默 0 笔的故障面；并在 `run_backtests` 末尾加过半策略报错熔断（>50% 策略 error 时抛 RuntimeError），不再静默产出空结果污染 handoff。(5) 新增环境变量 `BACKTEST_CONFLUENCE=1`/`BACKTEST_REGIME_SOFT=1`（等价对应 CLI，CLI 优先），供 GitHub Actions/定时任务不改命令启用质量门槛。
- [文档] 更新 `docs/backtest-optimization-design.md`：新增第 8 章记录 P1-1/P1-2/P1-3/P1-4 的开关、ensemble 重构、合流过滤三档门控 A/B 结论与 numba 加固。
- [修复] WFO 回测确定性与出场参数 bug：(1) `compute_dynamic_exit_returns`（生产）与 `compute_dynamic_exit_returns_p`（Exp3 副本）改用**位置置换**计算动态退出收益——排序后全程用 0..n-1 位置索引、返回按 inv_order 逆置换还原行序，修复 df 原始 index 非唯一（多次 concat/merge 后常见）时 `out.reindex(df.index)` 按重复标签错位、导致 dyn_ret 的 NaN 模式跨参数组合随机漂移、Exp3 sweep 同参数交易数不一致（日志"交易数不一致…结果不可信"）的根因；构造非唯一 index 数据验证 9 个 (atr,trail) 组合交易数完全一致且行序正确对齐。(2) 修复生产函数移动止盈分支硬编码 `peaks[k]*0.95` 未使用 `trail` 参的 bug——导致 per-strategy 出场参数（STRATEGY_EXIT_PARAMS 的 0.97/0.92）在该分支失效，现改为 `peaks[k]*trail`，与 Exp3 参数化副本一致。
- [修复] `apply_industry_momentum` 行业动量合并健壮性：merge 前将 df 与 rank_df 的 date 统一 `normalize()` 到午夜（避免时间分量/dtype 差异导致按 (date,l1_code) 关联失败、ind_rank 全 NaN），merge 行数变化时告警，覆盖率日志同时输出最新交易日与全区间均值（WFO 验证全区间覆盖 78%，最新交易日边界为 0% 属数据新鲜度问题，不影响 IS/OOS 历史窗口）。
- [改进] WFO 框架接入行业动量过滤器并放宽窗口门槛：`backtest_wfo.py` `build_full_frame` 注入申万 L1 行业动量上下文（`load_industry_context`+`apply_industry_momentum` → ind_rank 列，缺失放行），IS 训练（组件串行 + 其余并行）与 OOS 评估（`_trade_returns`）均透传 `industry_filter=True`，`_backtest_single` 新增 `industry_filter` 参数（主回测验证为最强过滤器：胜率 55.5%→59.0%、回撤 80%→68%）；`WFO_MIN_OOS_TRADING_DAYS` 由 60 降到 30（6M 激活段约 120 交易日，旧门槛叠加 regime 过滤后常只剩 40~50 日，导致 4 窗口中 3 个被排除、聚合只靠单窗口易过拟合，降门槛后 ≥3 窗口可入样且仍排除 1 天残段）；`_compute_ensemble_weights` 容错改为**排除**报错/胜率≤0 的无效组件后对有效组件按胜率归一（而非静默赋权重 0），修复组件在某窗口失败时权重塌缩（如 ma_crossover=0.000）导致 ensemble 打分被拉低的问题。
- [修复] 数据源与分析链路稳定性治理：(1) 搜索引擎熔断快速失败——`src/search_service.py` `BaseSearchProvider` 新增 per-provider 熔断器，配额/鉴权类错误（quota/usage limit/401/403 等，覆盖 Tavily "exceeds usage limit"）立即熔断 10 分钟、连续超时/网络错误 3 次熔断 60 秒，冷却期内 `is_available=False` 直接跳过不再每维度重试空等，冷却到期放行 half-open 探测、成功自动重置；修复原"所有 key 错误计数满即无条件重置"导致配额耗尽引擎被反复重试的问题。(2) 新闻标志如实标注——`src/core/pipeline.py` 情报搜索仅在 `total_results>0` 时才生成 `news_context`，0 条结果保持 None，修复 `format_intel_report` 恒返回非空字符串（含"未找到相关信息"）导致 LLM 日志/`search_performed`/prompt 误判"包含新闻: 是"、把空情报当有新闻上下文喂给 LLM 的问题。(3) 实时行情去重——`data_provider/base.py` `get_realtime_quote` 增加 30s 短 TTL 去重缓存（原方法拆为 `_fetch_realtime_quote`），同股在一次分析流程内被 pipeline 与 fundamental valuation 块重复请求时复用结果，消除 tushare+tencent 双轮重复打源，`use_cache=False` 可强制刷新。
- [修复] `scripts/backtest_5y_23strategies.py` handoff 内存与 pandas 警告：回测完成后立即 `del df_bt`（580 万行级切片），handoff spawn `main.py` 子进程前显式释放 `df_week`/`val_results`/`df_bench_index` 等大对象并 gc，缓解父进程持有大 DataFrame 导致子进程 0 可用物理内存、全程 swap 的问题；修复 pandas FutureWarning——`rzye_chg5`/`holder_chg` 的 `pct_change` 显式传 `fill_method=None`（不再依赖默认 ffill），`_merge_aux_by_date` 补 NaN 列显式 `float64` dtype 消除空条目 concat 的 all-NA 警告。
- [修复] `scripts/backtest_5y_23strategies.py` 修正策略侧绩效口径并加严 5 日验证 handoff 门槛：(1) 组合级指标（总收益/年化/最大回撤/夏普）改用**日历日等权组合复利净值**口径——新增 `calc_portfolio_metrics`，把每笔信号持有期收益均匀分摊到持有期交易日、按日历日对齐取当日持仓等权平均作为组合日收益，再 cumprod 复利（与基准 `compute_benchmark_metrics` 日频口径一致）；修复旧口径对重叠选股信号简单累加导致的总收益虚高、最大回撤在累加曲线上算且被 clip 成 100% 假象、年化按串行笔数折算失真的问题（逐笔指标胜率/盈亏比/期望/凯利保留，`calc_metrics` 新增可选 `dates` 参数，不传则回退旧口径，`backtest_wfo*.py` 等旧调用不受影响）。(2) 小样本策略（`MIN_TRADES_FOR_RANKING=30` 笔以下）标记 `small_sample`，期望值排名时统一后置、不参与头部 Top-N 验证与跨策略平均（平均胜率/总收益/年化/跑赢基准统计改用样本充足策略），避免 1~10 笔策略（100% 胜率、虚高年化）污染排名与汇总。(3) 5 日验证自动送入主程序实盘分析加门槛（`get_top_stocks_by_win_rate`）：仅当验证期持仓收益 > 0 **且** 由历史样本充足（≥`HANDOFF_MIN_STRATEGY_TRADES=30` 笔）策略命中数 ≥ `HANDOFF_MIN_STRATEGIES=3`（共振门槛）才 handoff，被过滤股票记录原因日志，避免把验证期已亏损（如 -2.81%）或仅 1~2 个小样本策略命中的股票自动实盘化。
- [新功能] 新增本地 PostgreSQL Tushare 数据仓库运行时兜底数据源 `data_provider/pg_cache_fetcher.py`（`PgCacheFetcher`，默认优先级 90 末位兜底，配置 `PG_HOST`+`PG_PASSWORD` 后自动注册，未配置则不启用）：在线数据源因权限/网络/限流失败时，用本地 `tushare` 库兜底三类数据——筹码分布读 `tushare_cyq`（接入 `get_chip_distribution` 链）、个股资金流读 `tushare_moneyflow`（大单+特大单净额为主力净流入、近 5/10 日全单净额累计，万元换算为元，接入 `get_capital_flow_context` 且覆盖在线源整体超时/异常路径）、**申万行业资金流榜**按 `tushare_stock_basic.industry` 聚合 `tushare_moneyflow.net_mf_amount` 与 `tushare_daily.pct_chg` 作为 capital_flow 板块资金流榜（在线 `stock_sector_fund_flow_rank` 失败）的兜底，支持 `sort_by` 参数按主力净流入（默认）或行业均涨幅排序，标注 `source=pg_sw_industry_fund_flow`/`rank_type`/`classification=sw_industry`。**口径区分**：该本地聚合是申万行业资金流排行，**不接入**市场复盘 `get_sector_rankings` 板块涨跌榜链路（该链路在线侧为东财/同花顺行业板块+概念板块，分类体系不同），`PgCacheFetcher.get_sector_rankings` 显式返回 None 跳过，避免申万行业名混入涨跌榜；全部 fail-open，PG 不可用/表缺失/查询异常均不影响主流程。
- [改进] `scripts/backtest_5y_23strategies.py` 回测 P2/P3 落地决策与 P3 能力交付：行业动量过滤设为默认开启（`--no-industry-momentum` 显式关闭，行业数据缺失时 ind_rank 缺失自动放行）；smart_money_flow 移除出策略注册表（实测胜率 37.8% 弱，函数保留标注废弃沿用 bull_trend 惯例，其 SQL 分层字段保留供 ML 特征）；新增入场时点参数 `--entry-timing {close,next_open}`（P3-13：`compute_indicators` 新增次日开盘入场前瞻收益列 `ret_{p}d_no`，`compute_dynamic_exit_returns` 参数化 entry_timing 且风险窗口/出场日与 close 口径一致以隔离纯入场价效应）；新增推荐组合优化 `--portfolio-opt`（P3-12：`optimize_portfolio` 按优先级贪心保留、申万 L1 行业 ≤2 只 + 总持仓 ≤10、无映射不受限）；新增 ML 信号过滤 `--ml-filter`（P3-11：新训练脚本 `scripts/train_signal_filter.py` 构建 34 特征/dyn_ret_3d>0 标签/时间切分 OOS/LightGBM 训练与十分位报告，工件 result/ml_filter_model.txt+meta.json，main 加载阈值过滤信号字典且工件缺失优雅跳过；因本机全量加载 OOM 采用分年切片+每分片独立子进程架构以规避已文档化的同进程随机信号抑制）。ML 实测（样本 2154，OOS 520）：AUC=0.5179 近似无预测力、十分位非单调，确认不默认启用——规则过滤器（尤其行业动量）已捕获可解释方差
- [改进] `scripts/backtest_5y_23strategies.py` 回测新增 P2 数据增强（行业动量 `--industry-momentum`、分组止损 `--per-strategy-exit` 可选启用，新策略默认注册）：行业动量——新增 `load_industry_context`（申万 L1 行业指数 20 日动量排名 + 成分映射，成分取当前口径并记录 point-in-time 局限）与 `apply_industry_momentum`（合并 `ind_rank` 列，NaN 放行），`_strategy_signal` 新增 `industry_filter` 参数仅保留动量前 3 行业信号；主力资金分层——`load_data` SQL 扩展大单/超大单/小单买卖与 `net_big_amount` 字段，新增 `sig_smart_money_flow`（连续 3 日主力净流入+散户净流出）；新增 `sig_rsi_bullish_divergence`（价格新低但 RSI 未创新低的底背离，打分满分 13 阈值 9）；分组止损——`compute_dynamic_exit_returns` 参数化 `atr_mult/trail`（默认值不变逐位等价），新增 `STRATEGY_EXIT_PARAMS` 族映射（趋势 3.0/0.97、反转 1.5/0.92、涨停 1.0/0.95）与后缀列选择逻辑。实测（2021-01~2026-08，25 策略）：行业动量单独启用胜率 55.5%→59.0%、年化 107.4%→119.5%、回撤 80.1%→68.4%（最强杠杆）；分组止损单独启用胜率降至 50.3%、期望 1.404（统一 (2.5,0.95) 全局最优更优，参数化能力保留但默认关闭）；三者叠加为陷阱（交易 11211→2036、胜率 44.5%，过滤器非正交叠加不可盲目组合）
- [改进] `scripts/backtest_5y_23strategies.py` 回测新增 P1 策略优化（`--regime-filter` 可选启用，默认关闭）：市况分族调度——`compute_market_ok` 新增 `regime` 列（bull=指数>MA60 且 MA20 五日斜率>0.2%；bear=指数<MA60 且 MA20 下行；其余 range），新增 `REGIME_STRATEGIES` 映射表全覆盖 23 策略（bull→趋势/突破族、range→反转/箱体族、bear→防御/基本面族，同一策略可属多市况），`_strategy_signal` 新增 `regime_filter` 参数按当前市况过滤策略，贯通回测/5 日验证/推荐链路；信号强度注册表——提取 6 个内联打分逻辑为独立 score helper（wonderful_9_turn/n_pattern/limit_up_pullback/monthly_macd_20ma/multi_ma_resonance/wave_theory，阈值判定不变经等价性测试验证），新增 `STRATEGY_SCORE_FUNCS` 归一化强度注册表（8 策略）与 `_signal_strength` helper，`get_next_day_recommendations` 新增 `strength_weighted` 参数（默认 False 保持旧排序以兼容 `backtest_wfo.py` 每日选股，开启时评分改为 Σ(胜率×强度) 并按加权分排序）；动态仓位管理——新增 `compute_position_size`（半凯利 × ATR 波动率调整，上限 25%），推荐输出新增 `position_pct` 字段。实测（2021-01~2026-08，23 策略，同 enhanced+共振≥2 基座）：--regime-filter 使交易数 9056→4940（-45%）、平均胜率 56.6%→57.1%、期望 2.250→3.340（+48%）、最大回撤 78.3%→67.5%（-10.9pct），以信号量换取单笔质量与回撤显著改善；强度加权经真实数据预测力检验（pooled Spearman=-0.006 无预测力、最新交易日两排序 Top-10 一致）确认不默认开启
- [文档] 新增回测系统优化方案详细设计文档 `docs/backtest-optimization-design.md`：基于 WFO Exp2/Exp3/Exp4 实验结论与 PostgreSQL 数据库表分析，覆盖 13 项优化（P0: enhanced regime + 共振门槛 + 市值范围放宽；P1: 市况分族调度 + 信号强度加权 + 动态仓位管理；P2: 行业动量策略 + 量价背离策略 + 主力资金分层信号 + 动态止损 per-strategy；P3: ML 信号过滤 + 投资组合优化 + 日内择时），含完整数据库依赖矩阵与影响范围分析
- [文档] 新增回测系统优化实施路线图 `docs/backtest-optimization-roadmap.md`：四阶段渐进式实施计划，含每步代码改动位置、验证命令、验收标准与回滚检查清单，附带分支策略与 CLI 参数兼容性设计
- [改进] `scripts/backtest_5y_23strategies.py` 回测 P0 优化并设为默认行为：默认启用增强市场环境 regime（`compute_market_ok` 新增 `market_ok_enh` 列，严格条件 OR (指数>MA20 且 MA20 上行)，`_entry_mask`/`_apply_cooldown`/`_strategy_signal`/`run_backtests` 贯通 `enhanced` 参数）与同股同日多策略共振过滤（新增 `_apply_resonance`，逐行累加各策略布尔信号，命中数 < N 的统一置 False）；旧行为通过 `--strict`（严格 regime）与 `--resonance 1`（关闭共振）显式回退；`MAX_CIRC_MV_W` 从 500 亿放宽至 800 亿（实测 MIN 提至 100 亿会使平均胜率 54%→46.3%、信号量 -70%，故维持 20 亿下限）；`_backtest_single` 改为接受预计算信号 `sig` 参数（None 时内部现算，保持 `backtest_wfo.py`/`backtest_wfo_experiments.py`/`exp1_w4_diagnostic.py` 向后兼容）。5 年回测实测（2021-01~2026-08，23 策略）：resonance≥2 平均胜率 54.0%→57.6%，enhanced+resonance≥2 期望 2.199→2.303、年化 76.7%→117.5%（多策略均值口径）；端到端验证回测→5 日验证→推荐→`main.py` 个股分析链路跑通
- [改进] `scripts/backtest_5y_23strategies.py` 回测接入 6 张 Tushare 扩展表做数据增强并按策略族差异化过滤：`load_data` 直接 JOIN 筹码分布 `tushare_cyq`（获利盘/平均成本/90%集中度）、`tushare_daily_basic` 扩展字段（自由流通换手率/PS(TTM)/股息率/总市值）、融资余额 `tushare_margin_detail`（5 日变化率）；新增 `load_signal_aux` 按公告日 as-of 对齐股东人数环比（`tushare_stk_holdernumber`）、业绩预告方向（`tushare_forecast`）、质押比例（`tushare_pledge_stat`，end_date+30d 防前视）、机构龙虎榜净买（`tushare_top_inst`，5 日滚动）；新增 `STRATEGY_MASKS` 按策略族质量过滤（standard=换手率 1~12%+获利盘 20~80%+质押≤40%+股息率≥1%；washout/limitup_break/limitup_pullback/chase 差异化组合，数据缺失放行），回测/5 日验证/推荐统一走 `_strategy_signal`；新增 5 个数据驱动策略 washout_break（胜率 56.0%）、low_profit_hold（66.0%）、holder_conc_break（48.1%）、inst_smart_break（49.8%）、fc_pos_break（49.5%）；dragon_head 增加获利盘≤90%+筹码集中度<15% 条件（胜率 33.6%→47.7%）；废弃胜率无法通过过滤提升的 bull_trend（41.9%）与 5 年零信号的 shrink_pullback。5 年回测（2021-01~2026-08）策略平均胜率 47.5%→约 51.8%（基于 2026-08-13 实测，主策略族胜率 +2~4pp，信号样本相应收紧）。
...
- [改进] 回测动态出场参数按 WFO 实验回写：`scripts/backtest_5y_23strategies.py` 的 `compute_dynamic_exit_returns` 与 `scripts/backtest_wfo.py` 的 `_find_exit_day_offset` 的 ATR 止损倍数 2.0→2.5、移动止盈回撤系数 0.92→0.95（`scripts/backtest_wfo_experiments.py` 参数化副本默认值同步）；依据 `result/wfo_experiments_20260820.md` Exp3 sweep，该组合胜率 48.9%→53.1%、期望 +0.664→+1.119、年化 35.2%→59.4%，且随参数变宽胜率单调提升（3x3 网格,5 次运行方向一致）。
...
- [文档] 新增 WFO 滚动向前回测方案设计文档 `docs/wfo-backtest-design.md`：4Y 训练/1Y 验证/6M 步长窗口网格与每日激活映射（评估口径=生产口径）、固化逻辑与滚动变量边界（best_p 选期改按 expectation、入选策略 Top-8、组合权重按 IS 胜率）、每日选股部署工件与 6M 日历重训练时机。
- [新功能] 新增 WFO 滚动向前回测脚本 `scripts/backtest_wfo.py`：`--mode research|deploy|daily` 三入口；`build_window_grid` 按 4Y 训练/1Y 验证/6M 步长吸附交易日生成窗口网格（锚定 2021-01-01 起）；`run_is_selection` 在 IS 切片上逐策略复用 `_backtest_single(select_period_by="expectation")` 打分并选 Top-8 + 每策略最优持有期 + 组合权重；`run_oos_eval` 用冻结选择在 val 切片跑同一管线并输出 IS→OOS 衰减与策略集 churn；`aggregate_oos` 按激活映射按天去重聚合 OOS 并与沪深 300 对比、输出 Spearman 排序相关性；`save/load_deployed_model` 读写部署工件 `result/wfo_model_latest.json`（含 `is_total_trades` 供每日过滤）；`run_daily_select` 读工件 + 最近 ~1 年数据按 `get_next_day_recommendations` 同款打分输出每日选股；`--force-retrain` 显式触发重训，默认 6M 日历边界自动重训。配套最小侵入改动：`scripts/backtest_5y_23strategies.py` 的 `_backtest_single` 增加 `select_period_by` 参数（默认 `total_return` 保持原行为）、抽出 `_compute_ensemble_weights(component_results)` 供 `run_backtests` 与 WFO 共用组合权重计算。
