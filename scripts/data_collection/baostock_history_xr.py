...
log_file = os.path.join(log_dir, 'baostock_history_xr.log')
...
class DatabaseOperations:
    ...
    def init_schema(conn):
        ...
        with conn.cursor() as cursor:
            # 创建表
            cursor.execute("""
                           CREATE TABLE IF NOT EXISTS baostock_daily_history_xr
            ...
            # 创建索引
            cursor.execute("""
                           CREATE INDEX IF NOT EXISTS idx_baostock_daily_history_xr_code
                               ON baostock_daily_history_xr (code)
                           """)

            cursor.execute("""
                           CREATE INDEX IF NOT EXISTS idx_baostock_daily_history_xr_date
                               ON baostock_daily_history_xr (date)
                           """)
    ...
    def add_column_comments(client):
        ...
        try:
            for column, comment in column_comments:
                comment_query = sql.SQL("COMMENT ON COLUMN baostock_daily_history_xr.{} IS %s").format(
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                CREATE TEMPORARY TABLE {temp_table_name} (
                    LIKE baostock_daily_history_xr INCLUDING ALL
                ) ON COMMIT DROP
                """)
                ...
                cursor.execute(f"""
                INSERT INTO baostock_daily_history_xr ({', '.join(expected_columns)})
...
class DateManager:
    ...
    def get_date_range(conn, code, start_date, end_date):
        ...
        # 快速检查股票是否存在于数据库
        with conn.cursor() as cursor:
            cursor.execute("SELECT EXISTS(SELECT 1 FROM baostock_daily_history_xr WHERE code = %s LIMIT 1)", (code,))
    ...
    def get_daily_data(code, name, ipo_date, out_date, type, status, start_date, end_date):
        ...
        while retry_count < max_retries:
            try:
                ...
                retry_count += 1
                if retry_count >= max_retries:
                    raise RuntimeError(f"{code} 获取数据失败，重试耗尽: {e}") from e
                time.sleep(2 ** retry_count)

        raise RuntimeError(f"{code} 获取数据失败，重试耗尽")
...
def run_stock_workers(args, stock_list, stock_latest_dates=None):
    ...
    for worker_id in range(worker_count):
        worker = threading.Thread(
            ...
            name=f"baostock-worker-{worker_id}",
        )
        worker.start()
...
def main(args):
    ...
    try:
        ...
        for code, inserted, error in run_stock_workers(args, stock_list, stock_latest_dates):
            completed_count += 1
            if error:
            ...
            if completed_count % 100 == 0 or completed_count == len(stock_list):
                logger.info(f"进度: {completed_count}/{len(stock_list)} ({100 * completed_count / len(stock_list):.1f}%)")
