...
工作流（三重过滤）：
  1. 通达信MCP（tdx_wenda_quotes） → 技术面 + 资金面验证
...
def normalize_code(raw: str) -> Dict[str, str]:
    ...
    # BaoStock格式: sh.600148 / sz.002154 → 转换为 600148.SH / 002154.SZ
    m_bs = re.match(r"^(SZ|SH|BJ)\.(\d{6})$", raw)
    if m_bs:
        raw = f"{m_bs.group(2)}.{m_bs.group(1)}"
